"""Fixtures for the planner eval corpus.

The eval runs every test on BOTH the floor (``local`` = Qwen via pi) and the
ceiling (``claude`` = Opus): if the local floor produces conforming, correctly
-shaped plans, the cloud ceiling will too. Each rig is a real endpoint, so live
execution is OPT-IN per rig via ``GRINDSTONE_EVAL_RIG`` (a comma list). A
parametrized rig not named there is skipped, so:

  * default (unset)               -> every eval is skipped (CI / collection only)
...
#: The rigs the eval parametrizes over: the local floor and the cloud ceiling.
EVAL_RIGS: tuple[str, ...] = ("local", "claude")

#: Env var naming which rigs to actually drive live (comma-separated). Absent or
#: empty = none (collection-only); the parent sets it to run the live baseline.
