...
if [[ -n "$workdir" ]]; then
  ...
  if [[ "$purpose" == "closeout" ]]; then
    ...
    # decision.json, no self-validate loop. Vision is load-bearing: pi's read-only tool
    # set includes `read`, which views images via the local vision endpoint, so the
    # :8080 server MUST have the mmproj tower for the planner to judge rendered UI.
